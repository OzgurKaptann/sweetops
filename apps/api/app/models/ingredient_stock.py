...
# The stock effect of a PHYSICAL COUNT: the signed delta that makes system on-hand
# equal what the manager actually found on the shelf.
#
# Deliberately its own type rather than a MANUAL_ADJUSTMENT, for the same reason
# the transfer legs are: analytics must be able to tell a counted discrepancy apart
# from a deliberate one-off correction. A count says "we looked, and reality
# differs"; an adjustment says "we know this figure is wrong". Folding the first
# into the second loses the count's two system snapshots and makes shrinkage
# invisible. See app/models/inventory_stock_count.py.
MOVEMENT_STOCK_COUNT_ADJUSTMENT = "STOCK_COUNT_ADJUSTMENT"
...
# receiving branch is entitled to know why a crate turned up. A count carries its
# own reason for the same reason a manual adjustment does.
...
class IngredientStockMovement(Base):
    ...
    STOCK_COUNT_ADJUSTMENT is never written alone either: it carries a
    ``stock_count_id``, and a composite key pins it to that count's store and
    ingredient. It is the stock EFFECT of a physical count, and the count — which
    holds what was counted and what the system believed — is the event. A count
    that found the shelf correct has a zero delta and therefore NO movement at
    all; see app/models/inventory_stock_count.py.

    The transfer legs are never written alone. They come in pairs, they carry a
    ...
    # Stock-count lineage — the count this movement is the stock effect OF. Null for
    # every other movement, and (by ck_movement_stock_count_link) mandatory for a
    # STOCK_COUNT_ADJUSTMENT. Unlike a transfer there is only ONE leg, so no
    # direction discriminator is needed: a single composite foreign key pins the
    # movement to the count's own store and ingredient.
    stock_count_id = Column(
        BigInteger, ForeignKey("inventory_stock_counts.id"), nullable=True, index=True
    )
    ...
    __table_args__ = (
        ...
        # ── Stock-count integrity ────────────────────────────────────────────
        # The movement's (count, store, ingredient) triple must BE the count's own.
        # Booking a Kadıköy count's correction against Beşiktaş's shelf, or against
        # a different ingredient, is not a bug review has to catch — it is a row
        # PostgreSQL will not store.
        ForeignKeyConstraint(
            ["stock_count_id", "store_id", "ingredient_id"],
            [
                "inventory_stock_counts.id",
                "inventory_stock_counts.store_id",
                "inventory_stock_counts.ingredient_id",
            ],
            name="fk_movement_stock_count_leg",
        ),
        # Biconditional, both halves load-bearing. A STOCK_COUNT_ADJUSTMENT with no
        # count is a correction with no evidence behind it — the very thing this
        # feature exists to stop. A stock_count_id on any OTHER type would let a
        # waste write-off masquerade as a counted discrepancy and disappear from
        # the waste report.
        CheckConstraint(
            "(movement_type = 'STOCK_COUNT_ADJUSTMENT') = (stock_count_id IS NOT NULL)",
            name="ck_movement_stock_count_link",
        ),
        # At most ONE movement per count. With the deferred trigger (exactly one
        # when the delta is non-zero, exactly zero when it is not) a count has
        # EXACTLY the movements its own delta demands.
        #
        # Note what ck_movement_quantity_positive already gives for free: a
        # STOCK_COUNT_ADJUSTMENT must have quantity = abs(delta), so a ZERO-delta
        # movement would need quantity = 0 and is structurally unrepresentable. The
        # zero-delta-writes-no-movement policy is not merely enforced — under this
        # delta rule it is the only thing the schema permits.
        Index(
            "uq_movement_stock_count",
            "stock_count_id",
            unique=True,
            postgresql_where=text("stock_count_id IS NOT NULL"),
        ),
