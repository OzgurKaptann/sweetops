"""
Smoke tests — fast sanity checks for every endpoint.
...
Protected staff endpoints (kitchen/*, owner/*) are exercised through
authenticated fixtures (owner_client / kitchen_client). Public endpoints
(health, menu, orders) use the plain module client.
"""
import uuid
...
def test_order_creation_invalid_status_value(kitchen_client):
    """Pydantic must reject unknown status strings (authenticated request)."""
    r = kitchen_client.patch("/kitchen/orders/99999/status", json={"status": "FLYING"})
    assert r.status_code == 422
...
def test_kitchen_orders_returns_dashboard(kitchen_client):
...
def test_kitchen_nonexistent_order_returns_404(kitchen_client):
    r = kitchen_client.patch("/kitchen/orders/999999999/status", json={"status": "IN_PREP"})
    assert r.status_code == 404


def test_kitchen_orders_requires_auth():
    """Unauthenticated kitchen access is rejected with 401."""
    r = client.get("/kitchen/orders/")
...
def test_owner_kpis_shape(owner_client):
    r = owner_client.get("/owner/kpis")
    assert r.status_code == 200
    body = r.json()
    assert "kpis" in body
    kpis = body["kpis"]
...
def test_owner_daily_sales_shape(owner_client):
    r = owner_client.get("/owner/daily-sales")
    assert r.status_code == 200
    body = r.json()
...
def test_owner_hourly_demand_shape(owner_client):
    r = owner_client.get("/owner/hourly-demand")
    assert r.status_code == 200
    body = r.json()
    assert "points" in body


def test_owner_top_ingredients_shape(owner_client):
    r = owner_client.get("/owner/top-ingredients")
    assert r.status_code == 200
    body = r.json()
...
def test_owner_stock_status_shape(owner_client):
    r = owner_client.get("/owner/stock-status")
    assert r.status_code == 200
    body = r.json()
...
def test_owner_forecast_shape(owner_client):
    r = owner_client.get("/owner/ingredient-forecast")
    assert r.status_code == 200
    body = r.json()
    assert "items" in body
    assert isinstance(body["items"], list)
...
def test_owner_insights_critical_alerts(owner_client):
    r = owner_client.get("/owner/insights/critical-alerts")
    assert r.status_code == 200
    body = r.json()
    assert "alerts" in body


def test_owner_insights_prep_time(owner_client):
    r = owner_client.get("/owner/insights/prep-time")
    assert r.status_code == 200


def test_owner_insights_trending(owner_client):
    r = owner_client.get("/owner/insights/trending-ingredients")
    assert r.status_code == 200


def test_owner_insights_combos(owner_client):
    r = owner_client.get("/owner/insights/popular-combos")
    assert r.status_code == 200


def test_owner_insights_value_summary(owner_client):
    r = owner_client.get("/owner/insights/value-summary")
    assert r.status_code == 200
    body = r.json()
    assert "weekly_revenue" in body


def test_owner_requires_auth():
    """Unauthenticated owner access is rejected with 401."""
