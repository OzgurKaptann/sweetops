...
interface AuthContextValue {
  ...
  logout: () => Promise<void>;
  // Children call this when a protected request returns 401 (session expired).
...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  if (phase === "forbidden") {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gray-100 text-center px-6">
  ...
  return (
    <AuthContext.Provider value={{ user, logout, reportUnauthorized }}>
      {children}
    ...
    <main className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      ...
      >
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900">Mutfak Girişi</h1>
        </div>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full py-2.5 rounded bg-amber-500 hover:bg-amber-600 text-white font-semibold text-sm disabled:opacity-60"
        >
