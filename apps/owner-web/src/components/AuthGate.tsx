...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  // Any 401 from a protected request → back to login with expiry notice.
  useEffect(() => {
    setUnauthorizedHandler(() => {
      setUser(null);
  ...
  return (
    <AuthContext.Provider value={{ user, logout }}>
      <div className="fixed top-3 right-3 z-50 flex items-center gap-2">
        {user && (
          <span className="text-xs text-gray-500 bg-white/80 rounded px-2 py-1 shadow-sm hidden sm:inline">
            {user.username} · {user.role}
          </span>
        )}
        <button
          onClick={logout}
          className="px-3 py-1 rounded bg-gray-800 text-white text-xs hover:bg-gray-900 shadow-sm"
        >
          Çıkış Yap
        </button>
      </div>
    ...
    <main className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      ...
      >
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900">İşletme Girişi</h1>
        </div>
